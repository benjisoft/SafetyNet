...
exports.newConfig =
    functions.region("europe-west1").https.onRequest((req, res) => {
      cors(req, res, () => {
        ...
        db.collection("alerts").add(data).then(() => { res.status(200); });
      });
    });

exports.disableAlerts =
    functions.region("europe-west1").https.onRequest((req, res) => {
      cors(req, res, () => {
                  ...
                  if (data.waitingForResponse == true) {
                    ...
                    data.recipients.forEach((recipient) => {
                      client.messages
                          .create({
                            body : `${
                                data.name} has not responded to a SafetyNet check. Please get in touch to check they are okay.`,
                            from : "+447723461766",
                            to : `+${recipient.number}`
                          })
                    ...
                    client.messages
                        .create({
                          body :
                              "Please confirm you are ok. You have one minute from now before your emergency contacts are alerted. Please respond with \"SAFE\" if you are safe.",
                          from : "+447723461766",
                          to : `+${data.usernumber}`
                        })
                        .then((message) => console.log(message.sid));
